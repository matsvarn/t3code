...
export const makeDevinTextGeneration = Effect.fn("makeDevinTextGeneration")(function* (
  ...
  const runDevinJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      // Same as chat sessions: pass the stored token so `authenticate` never
      // falls back to the PKCE browser flow.
      const devinApiKey = Option.getOrUndefined(
        yield* readDevinApiKey(environment).pipe(
          Effect.provideService(FileSystem.FileSystem, fileSystem),
          Effect.provideService(Path.Path, path),
        ),
      );
      // `--agent-type summarizer` runs Devin's no-tools agent, so generation
      // can never stall on a permission prompt.
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "Devin Agent returned invalid structured output.",
                cause,
              }),
            ),
        }),
